...
IF isUnfiltered
	THEN IF orderByAsc
		THEN RETURN QUERY SELECT row_to_json(row(id, title, price, description, pickup_location, return_location, post_date, start_date, end_date, is_avail, photo, category_name, owner)) 
			FROM itemlisting il ORDER BY il.price ASC, il.title ASC;
	ELSE RETURN QUERY SELECT row_to_json(row(id, title, price, description, pickup_location, return_location, post_date, start_date, end_date, is_avail, photo, category_name, owner)) 
		FROM itemlisting il ORDER BY il.price DESC, il.title ASC;
	END IF;
ELSE
	...
	IF orderByAsc
		THEN RETURN QUERY SELECT row_to_json(row(id, title, price, description, pickup_location, return_location, post_date, start_date, end_date, is_avail, photo, category_name, owner)) 
			FROM il_filtered il ORDER BY il.price ASC, il.title ASC;
	ELSE RETURN QUERY SELECT row_to_json(row(id, title, price, description, pickup_location, return_location, post_date, start_date, end_date, is_avail, photo, category_name, owner))
			FROM il_filtered il ORDER BY il.price DESC, il.title ASC;
	END IF;
...
SELECT "sort_by_price"(TRUE, FALSE, 'Mobile Devices', '2017-05-30', '2018-01-01', 'I');
SELECT "sort_by_price"(FALSE, FALSE, 'Mobile Devices', '2017-05-30', '2018-01-01', 'I');
SELECT "sort_by_price"(TRUE, TRUE, NULL, NULL, NULL, NULL);
SELECT "sort_by_price"(FALSE, TRUE, NULL, NULL, NULL, NULL);
