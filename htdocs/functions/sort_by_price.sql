...
IF orderByAsc
	THEN RETURN QUERY SELECT row_to_json(row(id, title, price, description, pickup_location, return_location, post_date, start_date, end_date, is_avail, photo, category_name, owner)) 
    	FROM filtered f ORDER BY f.price ASC, f.title ASC;
ELSE
	RETURN QUERY SELECT row_to_json(row(id, title, price, description, pickup_location, return_location, post_date, start_date, end_date, is_avail, photo, category_name, owner)) 
    	FROM filtered f ORDER BY f.price DESC, f.title ASC;
END IF;
DROP TABLE filtered;
END;
$BODY$
...
SELECT "sort_by_price"(TRUE, 'Mobile Devices', '2017-05-30', '2018-01-01', 'I');
SELECT "sort_by_price"(FALSE, 'Mobile Devices', '2017-05-30', '2018-01-01', 'I');
SELECT "sort_by_price"(TRUE, NULL, '2017-05-30', '2018-01-01', 'I');
SELECT "sort_by_price"(FALSE, 'Mobile Devices', NULL, '2018-01-01', 'I');
SELECT "sort_by_price"(TRUE, NULL, NULL, NULL, NULL);
SELECT "sort_by_price"(FALSE, NULL, NULL, NULL, NULL);
